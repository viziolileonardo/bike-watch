...
Data flow in `main()`: each enabled source's `search_*(cfg)` returns dicts with a common shape (`id` prefixed `source:`, title, description, location, price, image, …) → dedupe against `state.json` by id *and* by (title, price) fingerprint to suppress relistings → `classify()` returns `"alert"` (alert-term hit or one-edit fuzzy match), `"digest"` (kept for eyeballing if in price band), or None (dropped) → `is_hot()` upgrades cheap quality road bikes near the theft area to alerts → `blue_score()` colour-scores thumbnails (capped per sweep) → notifications (max 3 individual alerts + overflow summary) → `write_report()` regenerates the whole HTML report from state → atomic `save_state()`.
